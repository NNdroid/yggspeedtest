...
- **灵活的测速限制 (`-max-duration`, `-max-bytes`)**：可设定最大测速时长（如 10 秒）或最大流量消耗（如 `50MB`、`1.5MB`、`3kb`），避免大文件下载阻塞。
- **一键在线公共节点测速 (`-public`)**：拉取上游 `yggdrasil-network/public-peers` 仓库的国家级 Markdown 节点列表，跨区采样以便按路由质量排名。全部在线源不可用时回退到内置节点列表，并在日志中明确告警。
- **预检握手按协议实现**：TCP/TLS 真实握手、QUIC 带 ALPN 握手、WebSocket 完整 RFC 6455 握手（含 `Sec-WebSocket-Key`/`Version`，否则严格服务器拒绝升级）、SOCKS 走 `socks5://` 代理、KCP 只能做 UDP 可达性探测（当前 yggdrasil-go 已移除 KCP underlay，`core` 会把 `kcp://` 报成未知协议）。
...
- **参数合法性前置校验**：`-c` / `-streams` 小于 1、`-sort` 拼写错误、`-timeout` / `-route-timeout` 非正数、`-max-duration` 为负、`-max-bytes` 格式错误都会在启动时立即报错退出。`-sort` 拼错以前会静默退化成按速度排序，得到一份看起来正常但排错维度的榜单而毫无提示。
...
- **实时进度**：Server-Sent Events 推送 `run_started` / `peer_done` / `run_finished` / `schedule`；晚连接的浏览器会先收到一帧 `hello`，随后 15 秒一个心跳，代理不会掐断长连接。
- **内置排程器**：支持标准 5 字段 cron（`*`、`a-b`、`a,b,c`、`*/n`、`a-b/n`、月份与星期名字）和 `@every 30m` 之类的间隔写法，以及 `@hourly` / `@daily` / `@weekly` / `@monthly` / `@yearly` / `@midnight`。
  ...
  - `0 0 31 2 *` 这种不可能的日期会被扫描两年后放弃，不会让调度器空转。
...
**1. 单节点测试**
```bash
./yggspeedtest -peer "tls://ygg.mkg20001.io:443" -url "https://speed.cloudflare.com/__down?bytes=100000000"
```
...
**3. 一键公共节点批量测速并导出**
```bash
./yggspeedtest -public -url "https://speed.cloudflare.com/__down?bytes=100000000" \
...
/opt/yggspeedtest/runs.jsonl     # 每次运行的记录，每行一条
```
...
| `-url string` | `""` | 用于测速的 HTTP/HTTPS 下载目标 URL (必填) |
...
Web 端其余参数（节点列表、下载 URL、并发、流数、超时、排序、门槛、排程）都在配置文件或 UI 里改，`PUT /api/config` 会先校验、再原子落盘、再热生效，**配置文件和进程状态不会不一致**。
...
- **engine**：字节大小解析（含小数与 int64 溢出）、端口分配区间、节点去重、WebSocket Key 长度与随机性、CSV/Markdown 导出内容与 BOM、CSV 写错误上报、路由表轮询（假 admin socket，含空表超时与取消）、节点延迟按 key 匹配、速度/延迟门槛过滤（含「未测得则不通过」）、checkpoint JSONL 追加格式、稳定排序的并列保持、CJK 表头列宽对齐、UTF-8 截断、Peer URI 从 Markdown 中提取。
- **schedule**：`@every` 解析与最小间隔、全部速记写法、5 字段表达式的 26 个正反用例（越界、空列表项、`5-2`）、区间展开、真实星期数下的 `Match`、cron 日/星期 OR 规则、`NextAfter` 与不可能的日期（`0 0 31 2 *`）、调度器不重叠 / 忙时拒绝 / 跳过计数 / 停止语义。
- **store**：缺文件给默认值、往返序列化、非法 JSON 拒绝、覆盖参数与空列表语义、坏 duration 拒绝、覆盖不别名原切片、历史追加与截断、坏行跳过、原子写入不残留临时文件。
- **web**：`/healthz`、仪表盘自包含（断言无 `<link>` / `src=` / CDN 引用）、状态与配置形状、配置持久化与 7 类拒绝用例、被拒配置不落盘、运行生命周期（`202` → 历史记录 → 详情）、忙时 `409` 冲突、`404`、CSV 导出（含 BOM 与失败行）、删除幂等、历史最新在前、SSE 首帧 `hello`、排程热切换、摘要计算、run id 唯一性、`Cancel` 停止语义。
